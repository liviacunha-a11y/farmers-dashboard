...
export default function InactivePartners({ partners, farmerName }) {
  ...
  const visible = showAll ? partners : partners.slice(0, 10)
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-200 text-gray-500 text-xs uppercase tracking-wide">
            ...
            <th className="text-left py-2 pr-4">Última Atividade há</th>
            ...
            <tr key={p.org_id} className="border-b border-gray-100 hover:bg-gray-50">
              ...
              <td className="py-3 pr-4">
                <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                  Number(p.dias_inativo) > 30 ? 'bg-red-100 text-red-700' :
                  Number(p.dias_inativo) > 7 ? 'bg-yellow-100 text-yellow-700' :
                  'bg-green-100 text-green-700'
                }`}>
                  {p.dias_inativo >= 9000 ? 'Sem atividade' : `${p.dias_inativo} dias`}
                </span>
              </td>
      ...
      {partners.length > 10 && (
        ...
        >
          {showAll ? 'Ver menos ▲' : `Ver mais ${partners.length - 10} parceiros ▼`}
        </button>
      )}
    </div>
  )
